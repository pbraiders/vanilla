...
function ExtractRentsInfo(&$tRecord, $iDay)
{
    ...
    if (
        is_array($tRecord) && array_key_exists('day', $tRecord)
        ...
        if ($tRecord['day'] == $iDay) {
            $tReturn = array(
                (int)$tRecord['real'], (int)$tRecord['planned'], (int)$tRecord['canceled'], (int)$tRecord['max']
            );
        } //if( $tRecord['day']==$iDay )
...
function BuildDay($iDayName, $iDayNum, $iMonth, $iYear, $iReal, $iPlanned, $iCanceled, $iMax, $iState)
{
     ...
     ********************/
    if (
        !is_integer($iDayName) || !is_integer($iDayNum) || !is_integer($iMonth) || !is_integer($iYear) ||
