...
/**
  * function: RentsYearCount
  * description: Get the count of rents by year
...
function RentsYearCount( $sLogin, $sSession, $sInet, CDate $pDate, COption $pInterval)
{
     ...
     && IsStringNotEmpty($sInet) )
    {
        ...
        {
            // Prepare
            $sSQL = 'SELECT r.`year` AS "year", COUNT(r.`idreservation`) AS "count" FROM `'.PBR_DB_DBN.'`.`reservation` AS r';
            // Build interval
