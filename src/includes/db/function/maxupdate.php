...
/**
  * function: MaxUpdate
  * description: Update max rent value.
  * parameters:   STRING|sLogin        - login identifier
  *               STRING|sSession      - session identifier
  *               STRING|sInet         - concatenation of IP and USER_AGENT
  * CMaxRentPerMonthList|$pParameters  - list of value for each month
  ...
  */
function MaxUpdate( $sLogin, $sSession, $sInet, CMaxRentPerMonthList $pParameters)
{
    /** Initialize
     *************/
    $iReturn = -1;
     ...
     && IsStringNotEmpty($sInet) )
    {
        ...
        {
            // Prepare
            $sSQL = 'UPDATE `'.PBR_DB_DBN.'`.`config` SET `value`=:sValue WHERE `name` LIKE CONCAT_WS("_","max_rent",:sName)';
            $pPDOStatement = CDBLayer::GetInstance()->GetDriver()->prepare($sSQL);
            // Bind
            $sName = '';
            $sValue = '';
            $pPDOStatement->bindParam(':sName', $sName, PDO::PARAM_STR );
            $pPDOStatement->bindParam(':sValue', $sValue, PDO::PARAM_STR );
            // Execute
            foreach( $pParameters as $iKey=>$iValue )
            {
                $sName = strval($iKey);
                $sValue = strval($iValue);
                $pPDOStatement->execute();
                $iReturn = $iReturn + $pPDOStatement->rowCount();
            }//foreach( $tMonthMax as $key=>$value )
        }
