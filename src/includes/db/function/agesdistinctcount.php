...
/**
  * function: AgesDistinctCount
  * description: Get the distinct count of ages
  ...
  */
function AgesDistinctCount( $sLogin, $sSession, $sInet, CDate $pDate, COption $pInterval)
{
	/** Initialize
     *************/
    $iReturn = -1;
     ...
     && IsStringNotEmpty($sInet) )
    {
        ...
        {
            // Prepare
            $sSQL = 'SELECT 1 AS "label", count(r.`idreservation`) AS "count" FROM `'.PBR_DB_DBN.'`.`reservation` AS r WHERE r.`age`=1 AND r.`year`>=:iYear1 UNION SELECT 2 AS "label", count(r.`idreservation`) AS "count" FROM `'.PBR_DB_DBN.'`.`reservation` AS r WHERE r.`age`=2 AND r.`year`>=:iYear2 UNION SELECT 3 AS "label", count(r.`idreservation`) AS "count" FROM `'.PBR_DB_DBN.'`.`reservation` AS r WHERE r.`age`=3 AND r.`year`>=:iYear3';
