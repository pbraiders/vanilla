...
/**
  * function: ContactsMonthCount
  * description: Get the count of contacts by month;
  ...
  *                    'values' => array( years => array(months => count) )
  * author: Olivier JULLIEN - 2010-06-15
  */
function ContactsMonthCount( $sLogin, $sSession, $sInet, CDate $pDate, COption $pInterval)
{
	/** Initialize
     *************/
    $iReturn = -1;
     ...
     && IsStringNotEmpty($sInet) )
    {
        ...
        {
            // Prepare
            $sSQL = 'SELECT YEAR(c.`create_date`) AS "years", MONTH(c.`create_date`) AS "months", COUNT(c.`idcontact`) AS "count" FROM `'.PBR_DB_DBN.'`.`contact` AS c GROUP BY years,months HAVING years>=:iYear';
            ...
            while( $tRow = $pPDOStatement->fetch(PDO::FETCH_ASSOC) )
            {
                ...
                $iMonths = (integer)$tRow['months'];
                $iCount  = (integer)$tRow['count'];
                // Test first values for the year
                if( !isset( $tTemp[$iYears] ) )
                    $tTemp[$iYears] = array();
                // Add value
                $tTemp[$iYears][$iMonths] = $iCount;
                // Compute min and max
                if( $iIndex===0 )
                {
                    $iIndex=1;
