...
/**
  ...
  *              CUser|pUser    -  IN: identifier must be filled
  *                             - OUT: all user datas are filled
  * return: BOOLEAN - FALSE if an exception occures
  *         or
  *         INTEGER - >0 when the user is found. User Identifier.
        ...
        try
        {
            ...
            if( !empty($tabResult) && isset($tabResult[0]) && is_array($tabResult[0]) )
            {
                ...
                if( array_key_exists( 'user_id', $tabResult[0]) )
                    $pUser->SetIdentifier( $tabResult[0]['user_id'] );
                if( array_key_exists( 'user_name', $tabResult[0]) )
                    $pUser->SetUsername( $tabResult[0]['user_name'] );
                if( array_key_exists( 'user_state', $tabResult[0]) )
                    $pUser->SetState( $tabResult[0]['user_state'] );
                $iReturn = $pUser->GetIdentifier();
            }//if( !empty($tabResult) && isset($tabResult[0]) && is_array($tabResult[0]) )
        }
