...
/**
  ...
  * description: Get the count of rents by month;
  ...
  *                    'values' => array( <year => array( month => count)> )
  * author: Olivier JULLIEN - 2010-06-15
  */
function RentsMonthCount( $sLogin, $sSession, $sInet, CDate $pDate, COption $pInterval)
{
    /** Initialize
     *************/
    $iReturn = -1;
     ...
     && IsStringNotEmpty($sInet) )
    {
        ...
        {
            // Prepare
            $sSQL = 'SELECT r.`year` AS "year", r.`month` AS "month", COUNT(r.`idreservation`) AS "count" FROM `'.PBR_DB_DBN.'`.`reservation` AS r WHERE r.`year`>=:iYear GROUP BY r.`year`,r.`month`';
