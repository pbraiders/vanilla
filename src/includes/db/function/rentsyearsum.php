...
/**
  * function: RentsYearSum
  * description: Get the sum of real, planned and canceled rents by year
  ...
  *                    'values' => array( year => array( year, real, planned, canceled) )
  * author: Olivier JULLIEN - 2010-06-15
  */
function RentsYearSum( $sLogin, $sSession, $sInet, CDate $pDate, COption $pInterval)
{
    /** Initialize
     *************/
    $iReturn = -1;
     ...
     && IsStringNotEmpty($sInet) )
    {
        ...
        {
            // Prepare
            $sSQL = 'SELECT r.`year` AS "year", SUM(rent_real) AS "real", SUM(rent_planned) AS "planned", SUM(rent_canceled) AS "canceled" FROM `'.PBR_DB_DBN.'`.`reservation` AS r';
            // Build interval
            if( $pInterval->GetValue()>0 )
            {
                $sSQL .= ' WHERE r.`year`>=:iYear';
            }//Build interval
            $sSQL .= ' GROUP BY r.`year`';
