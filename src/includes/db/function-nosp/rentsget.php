...
function RentsGet( $sLogin, $sSession, $sInet, &$pDate, $iOffset, $iLimit)
{
        ...
        && CDb::GetInstance()->IsOpen()===TRUE )
    {
    	// Set error title
	    $sErrorTitle=__FUNCTION__;
	    $sErrorTitle.='('.$sLogin.','.$sSession.',[obfuscated],'.$pDate->GetRequestDay().','.$pDate->GetRequestMonth().','.$pDate->GetRequestYear().','.$iOffset.','.$iLimit.')';
        // Request
        if( CUser::GetInstance()->IsAuthenticated() )
        {
            ...
            {
                // Prepare
				$sSQL='SELECT 0 AS "reservation_id", IFNULL(SUM(r.`rent_real`),0) AS "reservation_real", IFNULL(SUM(r.`rent_planned`),0) AS "reservation_planned", IFNULL(SUM(r.`rent_canceled`),0) AS "reservation_canceled", IFNULL(MAX(r.`rent_max`),0) AS "reservation_arrhes", NULL AS "contact_lastname", NULL AS "contact_firstname", NULL AS "contact_phone", NULL AS "reservation_comment" FROM `'.PBR_DB_DBN.'`.`reservation` AS r INNER JOIN `'.PBR_DB_DBN.'`.`contact` AS c ON r.`idcontact`=c.`idcontact` WHERE r.`year`=:iYear AND r.`month`=:iMonth AND r.`day`=:iDay
UNION (SELECT r.`idreservation` AS "reservation_id", r.`rent_real` AS "reservation_real", r.`rent_planned` AS "reservation_planned", r.`rent_canceled` AS "reservation_canceled", r.`arrhe` AS "reservation_arrhes", c.`lastname` AS "contact_lastname", c.`firstname` AS "contact_firstname", c.`tel` AS "contact_phone", r.`comment` AS "reservation_comment" FROM `'.PBR_DB_DBN.'`.`reservation` AS r INNER JOIN `'.PBR_DB_DBN.'`.`contact` AS c ON r.`idcontact`=c.`idcontact` WHERE r.`year`=:iYear AND r.`month`=:iMonth AND r.`day`=:iDay LIMIT :iLimit OFFSET :iOffset)';
