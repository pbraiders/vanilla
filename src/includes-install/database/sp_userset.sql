 ...
 *************************************************************************/
DROP PROCEDURE IF EXISTS `_PBR_DB_DBN_`.`sp_UserSet`$$
CREATE PROCEDURE  `_PBR_DB_DBN_`.`sp_UserSet`(IN sLogin VARCHAR(45), IN sSession VARCHAR(200), IN sInet VARCHAR(255), IN sNewLogin VARCHAR(45), IN sNewPassword VARCHAR(40))
...
BEGIN
  ...
  DECLARE sErrorTitle TEXT DEFAULT ' IN sp_UserSet';
  ...
  SET sErrorTitle = CONCAT(sErrorTitle,'(',IFNULL(sLogin,'NULL'),',',IFNULL(sSession,'NULL'),',',IFNULL(sInet,'NULL'),',',IFNULL(sNewLogin,'NULL'),',',IFNULL(sNewPassword,'NULL'),')');
  ...
  -- Request --
  -- ------- --
  IF (sLogin IS NOT NULL) AND (LENGTH(sLogin)>0) AND (sSession IS NOT NULL) AND (LENGTH(sSession)>0) AND (sInet IS NOT NULL) AND (LENGTH(sInet)>0) AND (sNewLogin IS NOT NULL) AND (LENGTH(sNewLogin)>0) AND (sNewPassword IS NOT NULL) AND (LENGTH(sNewPassword)>0) THEN
  BEGIN
    -- Authentication --
    ...
    BEGIN
      -- Insert user --
      SET sErrorDescription = 'Insert user';
      INSERT INTO `_PBR_DB_DBN_`.`user` (`login`, `password`, `registered`, `role`, `last_visit`, `state`)      VALUES ( sNewLogin, sNewPassword, SYSDATE(), 1, NULL, 1);
