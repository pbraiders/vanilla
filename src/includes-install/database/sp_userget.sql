 ...
 *                                                                       *
 * Returns:                                                              *
 *         none or one record (user_id, user_name, user_state)           *
 *         or                                                            *
 ...
 *************************************************************************/
DROP PROCEDURE IF EXISTS `_PBR_DB_DBN_`.`sp_UserGet`$$
CREATE PROCEDURE  `_PBR_DB_DBN_`.`sp_UserGet`(IN sLogin VARCHAR(45), IN sSession VARCHAR(200), IN sInet VARCHAR(255), IN iIdentifier SMALLINT UNSIGNED)
...
BEGIN
  ...
  DECLARE sErrorTitle TEXT DEFAULT ' IN sp_UserGet';
  ...
  SET sErrorTitle = CONCAT(sErrorTitle,'(',IFNULL(sLogin,'NULL'),',',IFNULL(sSession,'NULL'),',',IFNULL(sInet,'NULL'),',',IFNULL(iIdentifier,'NULL'),')');
  ...
  BEGIN
    ...
    BEGIN
      ...
      SELECT u.`iduser` AS "user_id", u.`login` AS "user_name", u.`state` AS "user_state" FROM `_PBR_DB_DBN_`.`user` AS u WHERE u.`iduser`=iIdentifier;
    END;
    ELSE
    BEGIN
